 ...
 * (injected by the MissionTwinManager) and forward them to the Shadowing
 * Function.
...
public class MissionPhysicalAdapter extends ConfigurablePhysicalAdapter<MissionAdapterConfiguration> {
    ...
    private PhysicalAssetDescription buildPhysicalAssetDescription() {
        ...
        pad.getProperties().add(
                new PhysicalAssetProperty<>(MissionKeywords.SEVERITY_CODE_PROPERTY_KEY, cfg.getDefaultSeverityCode()));
        pad.getProperties().add(new PhysicalAssetProperty<>(MissionKeywords.CONFIRMED_SEVERITY_PROPERTY_KEY,
                cfg.getDefaultConfirmedSeverity()));
        pad.getProperties()
                .add(new PhysicalAssetProperty<>(MissionKeywords.PATHOLOGY_PROPERTY_KEY, cfg.getDefaultPathology()));
        pad.getProperties()
                .add(new PhysicalAssetProperty<>(MissionKeywords.PATIENT_ID_PROPERTY_KEY, cfg.getDefaultPatientId()));
        pad.getProperties()
                .add(new PhysicalAssetProperty<>(MissionKeywords.HOSPITAL_ID_PROPERTY_KEY, cfg.getDefaultHospitalId()));
        pad.getProperties().add(new PhysicalAssetProperty<>(MissionKeywords.CLINICAL_DETERIORATED_PROPERTY_KEY,
                cfg.isDefaultClinicalDeteriorated()));
        pad.getProperties()
                .add(new PhysicalAssetProperty<>(MissionKeywords.TIME_CALLED_PROPERTY_KEY, cfg.getDefaultTimeCalled()));
        pad.getProperties().add(
                new PhysicalAssetProperty<>(MissionKeywords.TIME_ON_SCENE_PROPERTY_KEY, cfg.getDefaultTimeOnScene()));
        pad.getProperties().add(
                new PhysicalAssetProperty<>(MissionKeywords.TIME_DEPARTED_PROPERTY_KEY, cfg.getDefaultTimeDeparted()));
        pad.getProperties().add(
                new PhysicalAssetProperty<>(MissionKeywords.TIME_HANDOVER_PROPERTY_KEY, cfg.getDefaultTimeHandover()));
        ...
        pad.getEvents().add(new PhysicalAssetEvent(MissionKeywords.VEHICLE_DISPATCHED_EVENT_KEY,
                MissionKeywords.CONTENT_TYPE_JSON));
        pad.getEvents().add(
                new PhysicalAssetEvent(MissionKeywords.ARRIVED_ON_SCENE_EVENT_KEY, MissionKeywords.CONTENT_TYPE_JSON));
        pad.getEvents().add(new PhysicalAssetEvent(MissionKeywords.CLINICAL_ASSESSMENT_EVENT_KEY,
                MissionKeywords.CONTENT_TYPE_JSON));
        pad.getEvents().add(new PhysicalAssetEvent(MissionKeywords.CLINICAL_DETERIORATION_EVENT_KEY,
                MissionKeywords.CONTENT_TYPE_JSON));
        pad.getEvents().add(
                new PhysicalAssetEvent(MissionKeywords.HOSPITAL_ASSIGNED_EVENT_KEY, MissionKeywords.CONTENT_TYPE_TEXT));
        pad.getEvents().add(new PhysicalAssetEvent(MissionKeywords.HANDOVER_COMPLETED_EVENT_KEY,
                MissionKeywords.CONTENT_TYPE_JSON));
    ...
    public void onMissionTelemetryReceived(MissionTelemetryPayload payload) {
        ...
        try {
            ...
            publishPhysicalAssetPropertyWldtEvent(new PhysicalAssetPropertyWldtEvent<>(
                    MissionKeywords.SEVERITY_CODE_PROPERTY_KEY, payload.severityCode()));
            publishPhysicalAssetPropertyWldtEvent(new PhysicalAssetPropertyWldtEvent<>(
                    MissionKeywords.CONFIRMED_SEVERITY_PROPERTY_KEY, payload.confirmedSeverityCode()));
            publishPhysicalAssetPropertyWldtEvent(
            ...
            publishPhysicalAssetPropertyWldtEvent(new PhysicalAssetPropertyWldtEvent<>(
                    MissionKeywords.HOSPITAL_ID_PROPERTY_KEY, payload.hospitalId()));
            publishPhysicalAssetPropertyWldtEvent(new PhysicalAssetPropertyWldtEvent<>(
                    MissionKeywords.CLINICAL_DETERIORATED_PROPERTY_KEY, payload.clinicalDeteriorated()));
            publishPhysicalAssetPropertyWldtEvent(new PhysicalAssetPropertyWldtEvent<>(
                    MissionKeywords.TIME_CALLED_PROPERTY_KEY, payload.timeCalled()));
            publishPhysicalAssetPropertyWldtEvent(new PhysicalAssetPropertyWldtEvent<>(
                    MissionKeywords.TIME_ON_SCENE_PROPERTY_KEY, payload.timeOnScene()));
            publishPhysicalAssetPropertyWldtEvent(new PhysicalAssetPropertyWldtEvent<>(
                    MissionKeywords.TIME_DEPARTED_PROPERTY_KEY, payload.timeDeparted()));
            publishPhysicalAssetPropertyWldtEvent(new PhysicalAssetPropertyWldtEvent<>(
                    MissionKeywords.TIME_HANDOVER_PROPERTY_KEY, payload.timeHandover()));
            ...
            System.err.println("EventBus error on telemetry: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void publishStateEvent(MissionTelemetryPayload payload) throws EventBusException {
        switch (payload.state()) {
            ...
            case MissionKeywords.STATE_TRANSPORTING -> {
                ...
                if (payload.clinicalDeteriorated()) {
                    publishPhysicalAssetEventWldtEvent(new PhysicalAssetEventWldtEvent<>(
                            MissionKeywords.CLINICAL_DETERIORATION_EVENT_KEY, payload));
                }

                if (!"null".equals(payload.hospitalId())) {
                    publishPhysicalAssetEventWldtEvent(new PhysicalAssetEventWldtEvent<>(
                            MissionKeywords.HOSPITAL_ASSIGNED_EVENT_KEY, payload.hospitalId()));
                }
            }
            ...
            default -> {
                /* STATE_TRIAGING: nessun evento preliminare */ }
        }
    }
    ...
    private void publishRelationshipInstances(MissionTelemetryPayload payload) throws EventBusException {
        ...
        // Istanziazione lasca targets_hospital quando l'ospedale viene assegnato o
        // aggiornato
        if (!"null".equals(payload.hospitalId())
    ...
    public void onIncomingPhysicalAction(PhysicalAssetActionWldtEvent<?> event) {
        ...
        if (MissionKeywords.ACTION_REROUTE_HOSPITAL.equals(event.getActionKey())
            ...
            try {
                ...
                // Generiamo la nuova istanza di relazione verso il nuovo ospedale di
                // destinazione
